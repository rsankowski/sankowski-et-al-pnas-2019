...
#protein coding genes data
                    counts <- read.csv("data/wt-veh-vs-stz-protein-coding-gene-counts.csv")
                    counts <- counts[!duplicated(counts$X),]
                    rownames(counts) <- counts$X
                    counts <- counts[,-1]
                    
                    metadata <- data.frame(row.names=colnames(counts), condition=c(rep('Veh',2), rep('STZ',3)))
...
#TE genes
                    counts <- read.csv("data/wt-veh-vs-stz-protein-coding-gene+transposable-elements-counts.csv", row.names = "Gene_name")[,-1]
                    ...
                    genes$family <- factor(genes$family, levels = c("ERV1", "ERVK", "ERVL", "TcMar", "TcMar-Tigger", "centr"))
